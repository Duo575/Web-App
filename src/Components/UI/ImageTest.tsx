...
export default function ImageTest() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'loaded': return 'border-green-500 bg-green-900/20';
    ...
    <div className="p-4 bg-gray-900 rounded-lg">
      ...
      <div className="mb-4 p-3 bg-gray-800 rounded">
        ...
        <div className="grid grid-cols-3 gap-2 text-xs">
          <div className={`p-2 rounded ${getStatusColor(imageStatus.codeSnippet)}`}>
            <div className="text-white">Code Snippet</div>
            <div className="text-gray-300">{getStatusText(imageStatus.codeSnippet)}</div>
          </div>
          <div className={`p-2 rounded ${getStatusColor(imageStatus.websiteView)}`}>
            <div className="text-white">Website View</div>
            <div className="text-gray-300">{getStatusText(imageStatus.websiteView)}</div>
          </div>
          <div className={`p-2 rounded ${getStatusColor(imageStatus.websiteLook)}`}>
            <div className="text-white">Website Look</div>
            <div className="text-gray-300">{getStatusText(imageStatus.websiteLook)}</div>
          </div>
        </div>
      </div>
      ...
      <div className="mb-4 p-3 bg-gray-800 rounded">
        ...
        <div className="space-y-1 text-xs">
          <div>
            <a href="/code-snippet.png" target="_blank" className="text-blue-400 hover:underline">
              /code-snippet.png
            </a>
          </div>
          <div>
            <a href="/website-view.png" target="_blank" className="text-blue-400 hover:underline">
              /website-view.png
            </a>
          </div>
          <div>
            <a href="/website-look.png" target="_blank" className="text-blue-400 hover:underline">
              /website-look.png
            </a>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <img 
            ...
            onError={(e) => console.error("❌ IMG tag: code-snippet.png failed:", e)}
          />
        </div>
        <div>
          ...
          <img 
            ...
            onError={(e) => console.error("❌ IMG tag: website-view.png failed:", e)}
          />
        </div>
        <div>
          ...
          <img 
            ...
            onError={(e) => console.error("❌ IMG tag: website-look.png failed:", e)}
          />
        </div>
      </div>
    </div>
